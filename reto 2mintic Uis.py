...
#La empresa te dice que ahora tienen una nueva política de costos porque ahora deberán declarar IVA. Por esto, el costo de envío de un paquete, aunque seguirá siendo $5, tiene estas nuevas consideraciones:
...
#Identifica las variables que se quieren definir.

#Recordar de qué manera se pueden recolectar datos por entrada estándar para definir variables.

#Recordar los operadores aritméticos para la definición de variables a partir de otras variables.

##Solución del reto

#Toma el código del reto anterior y elimina donde imprimes el volumen y el costo.

#Agrega un condicional if que verifique si la variable alto es mayor que 30, en caso de cumplirse, debes sumarle al costo un valor de 2000.

#Agrega un condicional if que verifique si la variable costo es mayor que 10000, en caso de cumplirse, debes sumarle al costo su IVA, ten en cuenta que el IVA es del 19%.

#mprime las respuestas:

#imprime la variable volumen

#Imprime la variable costo

alto=float(input("digita el alto "))
ancho=float(input("digita el ancho "))
profundo=float(input("digita lo profundo  "))
...
if (costo>10000 and alto>30):
     costo+=costo * 0.19
elif(costo>10000 and alto<30):
    costo+=costo*0.19
